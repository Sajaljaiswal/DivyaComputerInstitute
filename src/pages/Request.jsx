...
import { FiTrash2, FiPhone, FiMapPin, FiCalendar, FiRefreshCw, FiBook, FiSearch, FiMessageSquare } from 'react-icons/fi';
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-[2rem] p-20 text-center border-2 border-dashed border-slate-200">
            ...
            <p className="text-sm text-slate-400">New student inquiries will appear here.</p>
          </div>
        ) : (
          <div className="grid gap-4">
            {filteredRequests.map((request) => (
              <div 
                key={request.id} 
                className="bg-white rounded-2xl border border-slate-100 p-6 flex flex-col md:flex-row md:items-center gap-6 group hover:border-orange-200 hover:shadow-xl hover:shadow-orange-900/5 transition-all relative"
              >
                {/* Date Badge */}
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-slate-500 font-medium">
                    <a href={`tel:${request.phone}`} className="flex items-center gap-2 hover:text-orange-500 transition-colors">
                      <FiPhone className="text-orange-400" /> {request.phone}
                    </a>
                ...
                <div className="md:max-w-xs w-full bg-slate-50/50 p-4 rounded-xl border border-slate-100">
                  <p className="text-xs text-slate-500 italic leading-relaxed">
                    {request.description ? `"${request.description}"` : "No message provided."}
                  </p>
                </div>
