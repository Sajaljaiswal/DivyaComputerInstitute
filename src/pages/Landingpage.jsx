    ...
    <div className="min-h-screen bg-[#FFFBF7] font-sans text-slate-900 scroll-smooth">
      ...
      <nav className="flex items-center justify-between px-6 py-4 bg-white/90 backdrop-blur-lg border-b border-orange-100 sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <span className="text-xl font-black tracking-tight text-slate-800 leading-none">DIVYA</span>
            <span className="text-xs font-bold text-orange-500 tracking-[0.2em]">COMPUTER INSTITUTE</span>
          </div>
        </div>
        
        <div className="hidden md:flex items-center gap-8 text-sm font-bold text-slate-600">
          <a href="#courses" className="hover:text-orange-500 transition-colors">Courses</a>
          <a href="#about" className="hover:text-orange-500 transition-colors">Why Us</a>
          <a href="#faq" className="hover:text-orange-500 transition-colors">Help</a>
          <button 
        ...
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 items-center">
          <div className="z-10 text-center lg:text-left">
            ...
            <h1 className="text-5xl lg:text-7xl font-black leading-[1.1] mb-6 text-slate-900">
              Master the Digital <span className="text-orange-500">Universe.</span>
            </h1>
            <p className="text-lg text-slate-600 mb-10 leading-relaxed max-w-lg mx-auto lg:mx-0">
              From basic computing to high-end software development. Join Divya Computer Institute and turn your curiosity into a professional career.
            </p>
            ...
            <div className="bg-gradient-to-tr from-orange-400 to-amber-300 p-3 rounded-[2.5rem] shadow-2xl">
              <div className="bg-white rounded-[2rem] overflow-hidden p-6">
                 <div className="flex items-center gap-4 mb-6">
                    <div className="w-12 h-12 rounded-full bg-orange-100 flex items-center justify-center text-orange-600 font-bold">JD</div>
                    <div>
                        <p className="text-sm font-bold text-slate-800">Student Progress</p>
                        <p className="text-xs text-slate-400">Full Stack Web Development</p>
                    </div>
                 </div>
                 <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        <div className="h-16 bg-orange-50 rounded-xl border border-orange-100 flex items-center justify-center text-orange-500 font-bold text-xs uppercase text-center p-2">HTML/CSS Complete</div>
                        <div className="h-16 bg-orange-50 rounded-xl border border-orange-100 flex items-center justify-center text-orange-500 font-bold text-xs uppercase text-center p-2">JS Basics Done</div>
                        <div className="h-16 bg-slate-50 rounded-xl border border-dashed border-slate-200 flex items-center justify-center text-slate-400 font-bold text-xs uppercase text-center p-2">React Next</div>
                    </div>
                 </div>
              </div>
            </div>
      ...
      <section id="courses" className="px-8 py-24 bg-white">
        <div className="max-w-6xl mx-auto text-center">
            <h2 className="text-4xl font-black mb-4">Choose Your <span className="text-orange-500">Path.</span></h2>
            <p className="text-slate-500 mb-16 max-w-2xl mx-auto">Expert-led courses designed to take you from a beginner to a tech professional.</p>
            ...
            <div className="grid md:grid-cols-3 gap-8">
                <CourseCard 
                    ...
                    desc="Master Python, Java, and C++. Perfect for students looking to enter the world of coding." 
                />
                <CourseCard 
                    icon={<FiSmartphone />} 
                    color="bg-orange-500"
                    title="Web & App Design" 
                    desc="Learn modern UI/UX and Full Stack Web Development using React and Node.js." 
                />
                <CourseCard 
                    icon={<FiCpu />} 
                    color="bg-amber-500"
                    title="Computer Basics & Office" 
                    desc="Essential skills for every student: MS Office, Tally Prime, and Digital Literacy." 
                />
            </div>
        </div>
      </section>
      ...
      {/* --- Why Us Section --- */}
      <section id="about" className="px-8 py-24 bg-slate-900 text-white rounded-[3rem] mx-4 mb-24 overflow-hidden relative">
        <div className="absolute top-0 right-0 w-96 h-96 bg-orange-500/10 blur-3xl rounded-full"></div>
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-center">
            <div>
                <h2 className="text-4xl font-black mb-8 leading-tight">Why Divya Computer <br />Institute?</h2>
                <div className="space-y-6">
                    <Point icon={<FiCheckCircle className="text-orange-400" />} text="Industry-expert faculty with real-world dev experience." />
                    <Point icon={<FiCheckCircle className="text-orange-400" />} text="Latest curriculum updated for 2026 standards." />
                    <Point icon={<FiCheckCircle className="text-orange-400" />} text="Verified Certifications to boost your resume." />
                    <Point icon={<FiCheckCircle className="text-orange-400" />} text="Flexible timings for school and college students." />
                </div>
            </div>
            <div className="bg-white/10 p-8 rounded-3xl backdrop-blur-sm border border-white/10">
                ...
                <p className="text-slate-300 leading-relaxed">
                   We don't just teach software; we teach you how to build it. Our founder brings years of professional development experience to the classroom, giving students insights that books can't provide.
                </p>
            </div>
        </div>
      </section>
      ...
      <section id="faq" className="px-8 py-24">
        <div className="max-w-3xl mx-auto">
            <div className="flex flex-col items-center gap-3 mb-12 text-center">
                <div className="w-12 h-12 bg-orange-100 rounded-full flex items-center justify-center text-orange-500"><FiHelpCircle size={28}/></div>
                <h2 className="text-3xl font-black">Frequently Asked Questions</h2>
            </div>
            <div className="space-y-4">
                <FAQItem question="Do I need my own laptop?" answer="While we have a fully-equipped computer lab for every student, having your own laptop is recommended for home practice." />
                <FAQItem question="Are the certificates valid for Government jobs?" answer="Yes! Our certifications are recognized and valuable for both private and government job applications." />
                <FAQItem question="Is there any placement assistance?" answer="Absolutely. We help our advanced students with portfolio building, interview prep, and connecting with local tech firms." />
            </div>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-4 gap-12">
          <div className="col-span-1 md:col-span-2">
            <div className="flex items-center gap-2 mb-6">
              <div className="w-10 h-10 bg-orange-500 rounded-lg flex items-center justify-center font-bold text-white"><FiMonitor /></div>
              <span className="font-black text-xl tracking-tight">DIVYA INSTITUTE</span>
            </div>
            <p className="text-slate-500 text-sm leading-relaxed max-w-sm">Empowering the next generation of digital creators and professionals with quality education since 2000.</p>
          </div>
          <div className="space-y-4">
            <h5 className="font-bold text-slate-800 uppercase text-xs tracking-widest">Connect</h5>
            <p className="text-slate-500 text-sm">Mardah Bazaar, Ghazipur, UP</p>
            <p className="text-slate-500 text-sm">+91 9792257501</p>
          </div>
          <div className="space-y-4">
            <h5 className="font-bold text-slate-800 uppercase text-xs tracking-widest">Links</h5>
            <p className="text-slate-500 text-sm hover:text-orange-500 cursor-pointer transition-colors">Privacy Policy</p>
            <p className="text-slate-500 text-sm hover:text-orange-500 cursor-pointer transition-colors">Terms of Service</p>
          </div>
        </div>
...
function CourseCard({ icon, title, desc, color }) {
  return (
    <div className="p-8 rounded-[2rem] border border-slate-100 bg-white shadow-xl shadow-slate-100/50 hover:shadow-orange-100 hover:-translate-y-2 transition-all group text-left">
      <div className={`w-14 h-14 ${color} rounded-2xl shadow-lg flex items-center justify-center text-2xl text-white mb-6 group-hover:rotate-12 transition-transform`}>
        {icon}
      </div>
